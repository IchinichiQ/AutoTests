...
SEARCH_URL = BASE_URL + "/search/?q=%D0%BB%D0%B0%D0%BA"
...
def click_favorites_button_product_page(driver):
...
def test_add_from_product_page(driver, test_name):
    ...
    click_favorites_button_product_page(driver)
    open_favorites(driver)

    # Assert
    assert is_product_in_favorites(driver, product_page_title, product_page_price)

@pytest.mark.parametrize("test_name", ["Добавление товара в избранное со страницы поиска"])
def test_add_from_search(driver, test_name):
    # Arrange
    product, title, price = get_first_product_info_catalog(driver, SEARCH_URL)
...
@pytest.mark.parametrize("test_name", ["Удаление товара из избранного со страницы поиска"])
def test_remove_from_catalog(driver, test_name):
    # Arrange
    product, title, price = get_first_product_info_catalog(driver, SEARCH_URL)
    click_favorites_button(product)
